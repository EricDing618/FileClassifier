...
@njit
def classify(pathlist:list[str],file=None,notempty=False):
    if notempty: #字典不为空
        for item in file: #type(item)==list,type(item[0])==str,type(item[1])==str|list
...
class Ui_MainWindow(object):
    def start(self):
        try:
            ...
            if f:
                classify(array(self.a),array(list(f)),True)
            else:
